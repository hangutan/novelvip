...
import { random, translateFunc, TranslateType } from '@utils';

import styles from './HomeReviewQues.module.scss';
...
type HomeReviewQuesProps = {
	translate: TranslateType;
};

const HomeReviewQuestion = (props: HomeReviewQuesProps) => {
	...
	return (
		<>
			<div className={styles.title}>{translate('CONTRIBUTE IDEA')}</div>
			<section id="features" className={classnames(styles['screenshots-area'], 'ptb_100')}>
				<div className="container">
					<div className="row">
						<div className={classnames(styles.bl_btn)}>
							<button type="submit" className={classnames(styles.btn1, styles.active)}>
								{translate('Review')}
							</button>
							{/* <button type="submit" className={classnames(styles.btn1)}>
								{translate('Frequently asked questions')}
							</button> */}
						</div>
					</div>
					<div className={classnames(styles.bl_slide, 'row')}>
						<div className={classnames(styles['app-screenshots'], 'col-12')}>
							<Slider {...settings}>
								{features.map((feature) => (
									<div key={feature.key} className={classnames(styles['item-slide'])}>
										<div className={classnames(styles['item-slide__header'])}>
											<div className={classnames(styles['item-slide__display-flex'])}>
												<i className="icon-mt-language" />
												<span className={classnames(styles['item-slide__ml5'])}>
													{feature.date}
												</span>
											</div>
											<div className={classnames(styles['item-slide__display-flex'])}>
												...
												<i className="icon-mt-star" />
												<i className="icon-mt-star" />
												<i className="icon-mt-star_border" />
											</div>
										</div>
										<Image
											src={feature.image}
										...
										<div
											className={classnames(
												styles['item-slide__name'],
												styles['item-slide__mt10'],
											)}
										>
											{feature.name}
										</div>
...
export default HomeReviewQuestion;
